...
export const createServerJoinRoomRouteHandler = (sonolus, multiplayer) => async ({ req, res, localize, ctx }) => {
    ...
    const result = await webcrypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, signaturePublicKey, signatureBuffer, body);
    if (!result) {
        res.status(400).end();
        return;
    }
